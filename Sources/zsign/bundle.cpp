...
bool ZAppBundle::SignNode(JValue &jvNode)
{
    if (jvNode.has("folders"))
    {
        // Snapshot child pointers serially — JValue access on the shared parent
        // isn't thread-safe; each child is a disjoint subtree, safe in parallel.
        size_t nFolders = jvNode["folders"].size();
        std::vector<JValue *> folders;
        ...
        if (m_bParallel && nFolders > 1)
        {
            ...
            JValue **fp = folders.data();
            ZAppBundle *self = this;
            dispatch_apply(nFolders, DISPATCH_APPLY_AUTO, ^(size_t i) {
                if (!okp->load(std::memory_order_relaxed)) return;
                if (!self->SignNode(*fp[i])) okp->store(false, std::memory_order_relaxed);
            });
        ...
        else
        {
            for (size_t i = 0; i < nFolders; i++)
                if (!SignNode(*folders[i])) return false;
        }
    }

    if (jvNode.has("files"))
    {
        // Snapshot file paths serially, then sign in parallel — each iteration
        // uses its own ZMachO and only reads the shared (read-only) sign asset.
        size_t nFiles = jvNode["files"].size();
        ...
        if (m_bParallel && nFiles > 1)
        {
            std::atomic<bool> ok(true);
            std::atomic<bool> *okp = &ok;
            const string *pf = files.data();
            ZAppBundle *self = this;
            dispatch_apply(nFiles, DISPATCH_APPLY_AUTO, ^(size_t i) {
                if (!okp->load(std::memory_order_relaxed)) return;
                const string &szFile = pf[i];
                ZLog::PrintV(">>> SignFile: \t%s\n", szFile.c_str());
                ZMachO macho;
                if (!macho.InitV("%s/%s", self->m_strAppFolder.c_str(), szFile.c_str())) { okp->store(false, std::memory_order_relaxed); return; }
                if (!macho.Sign(self->m_pSignAsset, self->m_bForceSign, "", "", "", "")) { okp->store(false, std::memory_order_relaxed); return; }
            });
            if (!ok.load()) return false;
        }
        else
        {
            ...
            {
                const string &szFile = files[i];
                ZLog::PrintV(">>> SignFile: \t%s\n", szFile.c_str());
                ZMachO macho;
                if (!macho.InitV("%s/%s", m_strAppFolder.c_str(), szFile.c_str())) return false;
                if (!macho.Sign(m_pSignAsset, m_bForceSign, "", "", "", "")) return false;
            }
        }
    }
